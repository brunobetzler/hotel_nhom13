...
const BASE = "/api/src/controllers";

const ROUTES = {
  Room: "RoomController.php",
...
// }
async function apiGet(entity) {
  const res = await fetch(`${BASE}/${ROUTES[entity]}`, {
    credentials: "include" // Mandatory to maintain session
  });
  return res.json();
}
...
async function apiPut(entity, id, body) {
  const res = await fetch(`${BASE}/${ROUTES[entity]}?id=${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
    credentials: "include" // Mandatory to maintain session
...
async function loadRooms() {
  ...
  data.forEach((r, i) => {
    ...
    tr.innerHTML = `
      ...
      <td class="py-3 px-4 text-right">
        <button class="btn-edit mr-2 text-blue-600" data-id="${
          r.room_id
        }">Edit</button>
        <button class="btn-del text-red-600" data-id="${
          r.room_id
        }">Delete</button>
      </td>
    `;
  ...
  document.querySelectorAll(".btn-edit").forEach((btn) => {
    btn.onclick = async () => {
      ...
      const item = (await apiGet("Room")).find(
        (x) => String(x.room_id) === String(id)
      );
...
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("roomCount")) {
    apiGet("Room").then(
      (d) => (document.getElementById("roomCount").textContent = d.length)
    );
    apiGet("Customer").then(
      (d) => (document.getElementById("customerCount").textContent = d.length)
    );
    apiGet("Booking").then(
      (d) => (document.getElementById("bookingCount").textContent = d.length)
    );
  }
  if (document.getElementById("roomTableBody")) {
    ...
    document.getElementById("roomModalClose").onclick = () =>
      hideModal("roomModal");
    document.getElementById("roomModalCloseBottom").onclick = () =>
      hideModal("roomModal");
  }
});
